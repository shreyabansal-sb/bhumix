...
export default function ReviewRecord({ tehsildarId: _tehsildarId }: Props) {
  ...
  /** APPROVE: update status to 'approved', set approved_at */
  const handleApprove = async () => {
    ...
    try {
      ...
      // Navigate back to queue with success
  ...
  /**
   * REJECT: critical bug fix from PRD Section 2
   * 1. Delete the file(s) from Supabase Storage
   * 2. DELETE the land_records row (CASCADE deletes land_documents)
   * Data must NOT be retained — no status flag, actual deletion.
   */
  const handleReject = async () => {
    ...
    try {
      // Step 1: Delete all files from storage for this record
      if (documents.length > 0) {
        ...
        if (storageError) {
          ...
          // Continue with DB delete even if storage delete has issues
        }
      }

      // Step 2: DELETE the land_records row
      // ON DELETE CASCADE will automatically remove land_documents rows
              ...
              {!showRejectConfirm ? (
                ...
                <div className="bg-red-50 border border-red-200 rounded-xl p-6">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-red-800 font-semibold">Confirm Rejection</h3>
                      <p className="text-red-700 text-sm mt-1">
                        This will <strong>permanently delete</strong> the record and its uploaded document.
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      ...
                      {actionLoading === 'reject' ? 'Deleting...' : 'Yes, Reject & Delete'}
                    </button>
                    <button
